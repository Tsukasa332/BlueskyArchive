...
表示言語は英語を既定とし、英語と日本語をFrontendへ同梱する。選択値は他の表示設定と同様にbrowser localStorageへ保存し、documentの`lang`属性、表示文言、日付、相対時刻、accessibility labelを同時に切り替える。言語設定はBackendへ送信せず、archive recordを変更しない。
...
- `frontend/src/i18n.ts`: 言語型、既定言語、共通UI文言
- `frontend/src/TimelineCard.tsx`: 投稿・リポストcard
- `frontend/src/archive.ts`: archive・日付表示・route helper
...
Frontendではproduction build時のTypeScript検査に加え、英語が既定であること、日本語への切替、reload後の設定保持、document言語の更新を確認する。
...
| 言語 | 英語が既定で、日本語へ切り替えた状態がreload後も保持される |
